...
public class DataProvider {
    ...
    public static List<BaseItem> getInitialItems() {
        return getSubItems(new GroupItem(R.drawable.ic_caregiver,"root"));
    }
    ...
    public static List<BaseItem> getSubItems(BaseItem baseItem) {
        ...
        for (int i = 0; i < ITEMS_PER_LEVEL; ++i) {
            ...
            if (i % 2 == 0 && nextLevel != MAX_LEVELS) {
                item = new GroupItem(R.drawable.ic_caregiver,"Group " + Integer.toString(++groupNr));
                ((GroupItem) item).setLevel(nextLevel);
            } else {
                item = new Item(R.drawable.ic_caregiver,"Item " + Integer.toString(++itemNr));
            }
            result.add(item);
        }
        return result;
